...
import { Color, ColorPreference, isTopscorer, type PlayerColorState } from "../index";
...
const createPlayerSample = (
  ...
  colorDifference: differenceFor(Color.WHITE, ColorPreference.STRONG),
...
test("TS-1: checks_if_top_player", () => {
  expect(isTopscorer(createPlayerSample(2))).toBeTrue();
  expect(isTopscorer(createPlayerSample(1))).toBeFalse();
});

test("TS-1: exactly_half_is_not_a_top_player", () => {
  // "More than half of possible points" is strict.
  expect(isTopscorer(createPlayerSample(1.5, 3))).toBeFalse();
  expect(isTopscorer(createPlayerSample(2, 3))).toBeTrue();
});

test("TS-1: no_rounds_played_is_not_a_top_player", () => {
  expect(isTopscorer(createPlayerSample(0, 0))).toBeFalse();
});
